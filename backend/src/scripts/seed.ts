...
import { PathNode, IEdge } from '../models/PathNode';
import { Department } from '../models/Department';
import { env } from '../config/env';

// Original locations data from public/js/locations.js
const locationsRaw = [
  ...
  { "name" : "food technology dept", "displayName": "Food Technology Dept", "x": 80.27600, "y": 26.50390, "children": ["T", "paint tech dept"], "category": "academics", "icon": "🍎", "tags": ["food technology", "department", "academic", "विभाग"] },
...
const seed = async () => {
  try {
    ...
    console.log('MongoDB connected. Dropping collections...');
    ...
    // 1. Seed POIs & Departments
    for (const loc of locationsRaw) {
      const isRouting = loc.category === 'routing';
      const coordinates: [number, number] = [loc.y, loc.x]; // [longitude, latitude]
      ...
      if (!isRouting) {
        await POI.create({
          ...
          accessible: loc.name.includes('divyangjan') || loc.name.includes('ramp') || loc.name.includes('lift')
        });

        if (loc.category === 'academics') {
    ...
    console.log('POIs & Departments seeded.');
    ...
    for (const loc of locationsRaw) {
      ...
      if (loc.children) {
        for (const childName of loc.children) {
          const childLoc = locationsRaw.find(l => l.name === childName);
          if (childLoc) {
            const distance = calculateHaversine(loc.x, loc.y, childLoc.x, childLoc.y);
            edges.push({
              targetNodeId: childName,
              distance,
              grade: 0,
              stepFree: !loc.name.includes('stairs') && !childName.includes('stairs')
            });
          }
        }
      }

      await PathNode.create({
        nodeId: loc.name,
        displayName: loc.displayName || loc.name,
        coordinates,
        edges
    ...
    console.log('PathNodes seeded successfully.');
    process.exit(0);
  } catch (error) {
